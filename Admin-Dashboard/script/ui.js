...
class DatabaseConnector{
    ...
    async get() {
        let data = await fetch(`/kosina-api/${this.target}`,{
            method: "GET",
            headers: {
    ...
    async patch(body, id){
        let data = await fetch(`/kosina-api/${this.target}/${id}`,{
            method: "PATCH",
            headers: {
                "Content-Type": "application/json; charset=utf-8",
                "Authorization": `Bearer ${tokenChecker.getCookie("token")}`
...
class ModifyCategory extends EditPanel{
    ...
    start(target = null){
        ...
        if (this.modificationType === "edit") {
             console.log(this.modificationType); this.displayCurrentValue(target, inputDomElements); }
...
class ModifyProductModel{
    constructor(inputValues){
        ...
        console.log(this.name, this.price, this.categoryId, this.available)
    }
...
class LoginPanel{
    ...
    async submit(){
        let data = await fetch("http://localhost/kosina-api/login",{
            method: "POST",
            headers: {
                "Content-Type": "application/json; charset: utf-8"
            },
...
class TokenChecker{
    async checkToken(){
        let data = await fetch("http://localhost/kosina-api/return",{
            method: "GET",
            headers: {
                "Authorization": `Bearer ${tokenChecker.getCookie("token")}`
            }
    ...
    open(id){
        ...
        addImage.onclick = async () => {
            if (this.checkIfEmpty().includes("null")) {
                ...
                console.log("sada")
                if (!img64) { console.log("why", img64); }
                this.addImage(img64)
            } else {
